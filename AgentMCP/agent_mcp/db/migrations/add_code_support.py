...
def migrate_database():
    ...
    try:
        ...
        cursor.execute(
            "SELECT meta_value FROM rag_meta WHERE meta_key = ?", ("last_indexed_code",)
        )
        ...
        if cursor.fetchone() is not None:
            ...
            if current_dim and current_dim != EMBEDDING_DIMENSION:
                logger.warning(
                    f"rag_embeddings table uses {current_dim} dimensions but config uses {EMBEDDING_DIMENSION}."
                )
                logger.warning(f"To use {EMBEDDING_DIMENSION} dimensions, you need to:")
                logger.warning(
                    "1. Delete all existing embeddings: DELETE FROM rag_embeddings;"
                )
                logger.warning(
                    "2. Drop and recreate the table: DROP TABLE rag_embeddings;"
                )
                ...
                logger.warning(
                    "Note: The server will automatically handle this when dimension changes are detected."
                )
            elif create_sql and str(EMBEDDING_DIMENSION) in create_sql[0]:
                logger.info(
                    f"rag_embeddings table already uses {EMBEDDING_DIMENSION} dimensions."
                )
            else:
                logger.info("Could not determine current embedding dimensions.")
        else:
            logger.info(
                "rag_embeddings table does not exist. It will be created with correct dimensions on first run."
            )
...
if __name__ == "__main__":
    ...
    print("==================================")
    print(
        f"This will migrate your database to support code-aware RAG with {EMBEDDING_DIMENSION} dimensions."
    )
    print()
