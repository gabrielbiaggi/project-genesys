...
def get_agent_by_id(agent_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if row:
            ...
            if "capabilities" in agent_data and isinstance(
                agent_data["capabilities"], str
            ):
                try:
                    agent_data["capabilities"] = json.loads(agent_data["capabilities"])
                except json.JSONDecodeError:
                    logger.warning(
                        f"Failed to parse capabilities JSON for agent {agent_id}. Raw: {agent_data['capabilities']}"
                    )
                    agent_data[
                        "capabilities"
                    ] = []  # Default to empty list on parse error
            return agent_data
        return None
    except sqlite3.Error as e:
        logger.error(
            f"Database error fetching agent by ID '{agent_id}': {e}", exc_info=True
        )
        return None
    except Exception as e:
        logger.error(
            f"Unexpected error fetching agent by ID '{agent_id}': {e}", exc_info=True
        )
        return None
    finally:
        if conn:
        ...
        if row:
            ...
            if "capabilities" in agent_data and isinstance(
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to parse capabilities JSON for agent with token. Raw: {agent_data['capabilities']}"
                    )
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            if "capabilities" in agent_data and isinstance(
                ...
                try:
                    agent_data["capabilities"] = json.loads(
                        agent_data["capabilities"] or "[]"
                    )
                except json.JSONDecodeError:
                    agent_data["capabilities"] = []
...
def update_agent_db_field(agent_id: str, field_name: str, new_value: Any) -> bool:
    ...
    if field_name not in [
        ...
        logger.error(
            f"Attempted to update an invalid or unsupported agent field: {field_name}"
        )
        return False
    ...
    try:
        ...
        if field_name == "capabilities":
            ...
            field_name == "updated_at" and new_value is None
        ...
        safe_field_name = allowed_fields[
            field_name
        ...
        if cursor.rowcount > 0:
            ...
            logger.warning(
                f"Agent '{agent_id}' not found or field '{field_name}' update had no effect in DB."
            )
            return False
        ...
        if conn:
            conn.rollback()
        logger.error(
            f"Database error updating agent '{agent_id}' field '{field_name}': {e}",
            exc_info=True,
        )
        return False
    except Exception as e:
        if conn:
            conn.rollback()
        logger.error(
            f"Unexpected error updating agent '{agent_id}' field '{field_name}': {e}",
            exc_info=True,
        )
        return False
    finally:
