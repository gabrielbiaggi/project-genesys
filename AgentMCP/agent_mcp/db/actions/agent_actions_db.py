...
def log_agent_action_to_db(
    ...
    if details is not None:
        try:
            ...
            logger.error(
                f"Failed to serialize 'details' for agent action logging (agent: {agent_id}, action: {action_type}): {e}. Storing as string."
            )
    ...
    try:
        ...
        logger.error(
            f"Failed to log agent action '{action_type}' for agent '{agent_id}' (task_id: {task_id}) to DB: {e}"
        )
    except Exception as e:  # Original main.py line 268
        logger.error(
            f"Unexpected error logging agent action '{action_type}' for agent '{agent_id}': {e}",
            exc_info=True,
        )
