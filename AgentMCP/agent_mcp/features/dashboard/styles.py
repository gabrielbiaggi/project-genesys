...
def get_node_style(
    node_type: str, status: Optional[str] = None, agent_color: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    if (
        "color" not in node_style_properties and color
    ):  # Fallback to agent_color if no specific color set
        node_style_properties["color"] = color
