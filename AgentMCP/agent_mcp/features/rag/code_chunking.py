...
def _extract_javascript_entities(content: str) -> List[Dict[str, Any]]:
    ...
    patterns = [
        ...
        (
            r"(?:export\s+)?(?:const|let)\s+([A-Z]\w+)\s*=\s*(?:\([^)]*\)|[^=]+)\s*=>",
            "component",
        ),
    ]
...
def chunk_code_aware(
    ...
    if language_family == "python":
        ...
        return _chunk_generic_code(
            content, target_size, max_size, min_size, language_family
        )
    else:
...
def _chunk_python_code(
    content: str, target_size: int, max_size: int, min_size: int
) -> List[Tuple[str, Dict[str, Any]]]:
    ...
    while line_num < len(lines):
        ...
        if at_entity_start and current_chunk_size > min_size:
            ...
            for i in range(1, lookahead):
                if line_num + i < len(lines):
                    ...
                    if (
                        not future_line
                        or future_line.startswith("def ")
                        or future_line.startswith("class ")
                        or future_line.startswith("async def ")
                    ):
                        should_split = True
                        break
        ...
        if current_entity and line_num + 1 >= current_entity["start_line"]:
            ...
            if line_num + 1 >= current_entity.get(
                "end_line", current_entity["start_line"]
            ):
                entity_idx += 1
...
def _chunk_javascript_code(
    ...
    while line_num < len(lines):
        ...
        if (
            ...
            and line_num + 1 == entities[entity_idx]["start_line"]
        ):
            at_entity_start = True
...
def _chunk_generic_code(
    content: str, target_size: int, max_size: int, min_size: int, language: str
) -> List[Tuple[str, Dict[str, Any]]]:
    ...
    current_chunk_lines = []
    current_chunk_size = 0
    chunk_metadata = {"language": language, "section_type": "code", "line_range": None}
    ...
    for line_num, line in enumerate(lines):
        ...
        if current_chunk_size + line_size > max_size:
            ...
            if (
                ...
                and (not line.strip() or line.strip().startswith("//"))
            ):
                should_split = True
...
def create_file_summary(
    content: str, file_path: Path, entities: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
