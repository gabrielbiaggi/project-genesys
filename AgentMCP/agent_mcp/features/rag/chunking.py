...
def markdown_aware_chunker(
    ...
    if target_chunk_size <= 0 or min_chunk_size <= 0 or overlap_lines < 0:
        raise ValueError(
            "target_chunk_size, min_chunk_size must be positive, and overlap_lines non-negative."
        )
    ...
    for i, line_content in enumerate(lines):
        ...
        is_new_paragraph = (
            i > 0 and not lines[i - 1].strip() and line_content.strip() != ""
        )
        ...
        if (
            is_heading or is_new_paragraph
        ) and current_chunk_char_count >= min_chunk_size:
            ...
            current_chunk_char_count = (
                sum(len(line) + 1 for line in current_chunk_lines) - 1
            )  # +1 for newline
        else:
        ...
        if (
            current_chunk_char_count > target_chunk_size * 1.5
            and len(current_chunk_lines) > overlap_lines + 1
        ):  # Ensure we don't split too early if overlap is large
            ...
            if len(temp_chunk_str) > target_chunk_size * 1.5:
                ...
                char_count_before_current = (
                    sum(len(line) + 1 for line in lines_before_current) - 1
                )

                if (
                    char_count_before_current >= min_chunk_size
                ):  # If what we had *before* this line was substantial
                    chunks.append("\n".join(lines_before_current).strip())
                    current_chunk_lines = line_buffer_for_overlap[
                        :
