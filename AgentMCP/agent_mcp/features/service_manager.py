...
def get_venv_python_path() -> str:
    ...
    if not python_executable.exists():
        logger.warning(f"Python executável não encontrado em '{python_executable}'. Usando o python do sistema.")
...
def start_genesys_process() -> Dict[str, Any]:
    ...
    if g.genesys_process and g.genesys_process.poll() is None:
        return {"status": "already_running", "pid": g.genesys_process.pid, "message": "O processo do Genesys já está em execução."}

    try:
        python_executable = get_venv_python_path()
        genesys_script_path = Path(__file__).resolve().parent.parent.parent / "genesys_integration" / "genesys_server.py"

        if not genesys_script_path.exists():
            logger.error(f"Script do servidor Genesys não encontrado em: {genesys_script_path}")
            return {"status": "error", "message": "Script do servidor Genesys não encontrado."}
        ...
        return {"status": "started", "pid": process.pid, "message": "Servidor Genesys iniciado com sucesso."}
    except Exception as e:
        logger.error(f"Falha ao iniciar o processo do Genesys: {e}", exc_info=True)
        return {"status": "error", "message": f"Falha ao iniciar o processo do Genesys: {e}"}
...
def stop_genesys_process() -> Dict[str, Any]:
    ...
    if not g.genesys_process or g.genesys_process.poll() is not None:
        return {"status": "not_running", "message": "O processo do Genesys não está em execução."}
    ...
    try:
        ...
        return {"status": "stopped", "pid": pid, "message": "Servidor Genesys parado com sucesso."}
    except psutil.NoSuchProcess:
        logger.warning(f"Processo do Genesys com PID {g.genesys_process.pid} não encontrado. Pode já ter sido finalizado.")
        g.genesys_process = None
        return {"status": "not_found", "message": "Processo não encontrado, possivelmente já finalizado."}
    except Exception as e:
        ...
        if g.genesys_process:
            ...
            g.genesys_process = None
        return {"status": "error", "message": f"Erro ao parar processo: {e}. Forçado o encerramento."}
...
def get_genesys_status() -> Dict[str, Any]:
    ...
    if g.genesys_process and g.genesys_process.poll() is None:
        try:
            ...
            g.genesys_process = None
            return {"status": "stopped", "pid": None, "message": "Processo não encontrado, estado interno corrigido."}
