...
async def store_memory(
    source_ref: str, content: str, source_type: str = "pesquisa_autonoma"
):
            ...
            with open(req_path, "r") as f:
                for line in f:
                    ...
                    if line and not line.startswith("#"):
                        ...
                        topics.add(
                            f"Boas práticas e padrões avançados para a biblioteca Python '{package}'"
                        )
...
async def run_autonomous_research_loop(
    interval_seconds: int = 3600, *, task_status=anyio.TASK_STATUS_IGNORED
) -> NoReturn:
    """
    ...
    if not g.openai_client_instance and not genai:
        logger.error(
            "Nenhum cliente LLM (OpenAI/Gemini) configurado. Pesquisador autônomo desativado."
        )
        return
    ...
    while g.server_running:
        try:
            ...
            if response_text:
                logger.info(
                    f"Pesquisa sobre '{topic_to_research}' concluída. Armazenando conhecimento..."
                )
            ...
            logger.info(
                f"Ciclo de pesquisa concluído. Próximo ciclo em aproximadamente {sleep_duration / 60:.1f} minutos."
            )
