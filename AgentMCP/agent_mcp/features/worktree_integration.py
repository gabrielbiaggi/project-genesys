...
class WorktreeManager:
    ...
    def create_agent_worktree(
        self, agent_id: str, admin_token_suffix: str, config: WorktreeConfig
    ) -> Dict[str, Any]:
        """
        ...
        """
        if not self.enabled:
            return {"success": False, "error": "Worktree support not enabled"}
        ...
        try:
            ...
            create_result = create_git_worktree(
                path=worktree_path, branch=branch_name, base_branch=config.base_branch
            )
            ...
            if config.auto_setup:
                setup_commands = config.setup_commands or detect_project_setup_commands(
                    worktree_path
                )

                if setup_commands:
                    logger.info(
                        f"Running setup commands for {agent_id}: {setup_commands}"
                    )
                    setup_result = run_setup_commands(worktree_path, setup_commands)

                    if not setup_result["success"]:
                        logger.warning(
                            f"Setup failed for {agent_id}, but continuing: {setup_result}"
                        )
            ...
            worktree_info = {
                ...
                or detect_project_setup_commands(worktree_path),
    ...
    def cleanup_agent_worktree(
        ...
        try:
            ...
            if cleanup_result["success"]:
                ...
                logger.error(
                    f"Failed to cleanup worktree for agent {agent_id}: {cleanup_result['error']}"
                )
...
def create_agent_worktree(
    agent_id: str, admin_token_suffix: str, config: WorktreeConfig
) -> Dict[str, Any]:
    """
