...
async def system_usage_route(request: Request) -> JSONResponse:
    ...
    try:
        ...
        data = {
            ...
            "memory_percent": float(mem_line.split(':')[1].strip().split(' ')[0].replace('%', '')),
            "disk_percent": float(disk_line.split(':')[1].strip().split(' ')[0].replace('%', '')),
            "raw_text": result[0].text
        }
        return JSONResponse(data)
    except Exception as e:
        return JSONResponse({"error": f"Failed to parse usage data: {e}", "raw": result[0].text}, status_code=500)
...
async def kickstart_autonomy_route(request: Request) -> JSONResponse:
    ...
    try:
        ...
        if g.autonomous_research_task_scope is not None:
            return JSONResponse({"status": "already_running", "message": "Autonomous researcher is already active."}, status_code=200)

        if g.main_task_group is None:
            logger.error("Cannot start autonomous researcher: main task group not available.")
            return JSONResponse({"error": "Server in inconsistent state: Main task group not found."}, status_code=500)

        from ..features.autonomous_researcher import run_autonomous_research_loop

        researcher_interval = int(os.environ.get("MCP_AUTONOMOUS_RESEARCH_INTERVAL_SECONDS", "3600"))
        ...
        logger.info(f"Autonomous researcher task started on-demand with interval {researcher_interval}s.")

        return JSONResponse({"status": "started", "message": "Autonomous researcher has been activated."})

    except Exception as e:
        logger.error(f"Error in kickstart_autonomy_route: {e}", exc_info=True)
        return JSONResponse({"error": f"Failed to start autonomous researcher: {str(e)}"}, status_code=500)

routes.append(Route('/api/kickstart-autonomy', endpoint=kickstart_autonomy_route, name="kickstart_autonomy", methods=['POST', 'OPTIONS']))
