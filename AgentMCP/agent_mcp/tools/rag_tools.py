...
async def ask_project_rag_tool_impl(
    ...
    if not query_text or not isinstance(query_text, str):
        return [
            mcp_types.TextContent(
                type="text", text="Error: query text is required and must be a string."
            )
        ]
    ...
    logger.info(
        f"Agent '{requesting_agent_id}' is asking project RAG: '{query_text[:100]}...'"
    )
    ...
    try:
        ...
        logger.error(
            f"Unexpected error in ask_project_rag_tool_impl for agent '{requesting_agent_id}': {e}",
            exc_info=True,
        )
        return [
            mcp_types.TextContent(
                type="text",
                text=f"An unexpected error occurred while processing your RAG query: {str(e)}",
            )
        ]
    ...
    register_tool(
        ...
        input_schema={  # From main.py:1870-1881
            ...
            "properties": {
                "token": {
                    "type": "string",
                    "description": "Authentication token for the agent making the query.",
                },
                "query": {
                    "type": "string",
