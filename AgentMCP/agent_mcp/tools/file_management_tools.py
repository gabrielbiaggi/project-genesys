...
from ..core.auth import (
    get_agent_id,
)  # verify_token not strictly needed here if get_agent_id implies valid token
...
async def check_file_status_tool_impl(
    ...
    if not os.path.isabs(filepath_arg):
        ...
        if not agent_wd:
            ...
            logger.warning(
                f"Agent '{requesting_agent_id}' has no working directory set in g.agent_working_dirs. Using current server CWD as fallback for path resolution."
            )
            agent_wd = os.getcwd()
...
async def update_file_status_tool_impl(
    ...
    if (
        ...
        return [
            mcp_types.TextContent(
                type="text",
                text="Error: filepath and status are required and must be strings.",
            )
        ]
    ...
    if not os.path.isabs(filepath_arg):
        ...
        if not agent_wd:
            logger.warning(
                f"Agent '{requesting_agent_id}' has no working directory set. Using CWD for path resolution."
            )
            agent_wd = os.getcwd()
        resolved_abs_filepath = os.path.abspath(os.path.join(agent_wd, filepath_arg))
    else:
    ...
    if (
        ...
        and new_status != "released"
    ):  # Can always release, even if map is out of sync.
    ...
    if new_status == "released":  # main.py:1834-1841
        if resolved_abs_filepath in g.file_map:
            ...
            logger.info(
                f"Agent '{requesting_agent_id}' released file '{resolved_abs_filepath}'."
            )
            return [
                mcp_types.TextContent(
                    type="text",
                    text=f"File '{filepath_arg}' (resolved: {resolved_abs_filepath}) has been released.",
                )
            ]
        else:
        ...
        logger.info(
            f"Agent '{requesting_agent_id}' updated file '{resolved_abs_filepath}' status to '{new_status}'."
        )
        return [
            mcp_types.TextContent(
                type="text",
