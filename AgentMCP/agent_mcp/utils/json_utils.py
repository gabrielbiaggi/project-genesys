...
def sanitize_json_input(
    input_data: Union[str, bytes, Dict, List, Any],
) -> Union[Dict, List, Any]:  # Added bytes to input_data
    """
    ...
    if isinstance(input_data, bytes):
        ...
        try:
            ...
            raise ValueError(
                f"Input must be a JSON string, bytes, or Python object, got {type(input_data)}"
            )
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Inner regex/parse fallback failed during sanitization: {inner_e}"
            )
            pass
        ...
        logger.error(
            f"Aggressive JSON parsing failed: {e_cleaned}, cleaned data (excerpt): {error_excerpt}"
        )
        raise ValueError(
            f"Failed to parse JSON even after aggressive sanitization: {e_cleaned}"
        )


# Helper function for API request handling
# Original location: main.py lines 126-143
async def get_sanitized_json_body(
    request: Any,
) -> Union[Dict, List, Any]:  # 'request: Request' if Starlette is imported
    """
    ...
    try:
        ...
        ValueError
    ) as ve:  # Catch ValueError from sanitize_json_input or body decoding
